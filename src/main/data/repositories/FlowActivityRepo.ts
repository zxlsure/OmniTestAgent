...
export class FlowActivityRepo {
  create(projectId: string, activityType: string): FlowActivity {
    ...
    if (!result) throw new Error(`FlowActivity not found after insert: ${id}`)
    return result
  }
  ...
  countByProject(projectId: string): number { return (db().get('SELECT COUNT(*) as c FROM test_flow_activity WHERE project_id = ?', [projectId]) as any)?.c || 0 }
}
